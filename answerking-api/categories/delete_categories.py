...
def lambda_handler(event, context):
    ...
    try:
        response = table.delete_category(
            Key={'id': category_id},
            ConditionExpression='attribute_exists(id)'  # Prevents deleting non-existent categorys
        )
        return {
            'statusCode': 200,
            'body': json.dumps({'message': f' {category_id} deleted successfully.'})
        }
